...
CRITICAL - valid JSON only: never use a literal double-quote character (") inside any string value, in Institutional Analysis or anywhere else. If you want to reference a phrase from a central bank statement or other source, paraphrase it in your own words instead of quoting it directly - do not put it in quotation marks at all. A single unescaped double-quote anywhere in a string value breaks the entire JSON response and loses ALL data for this batch, not just that one field.
...
CRITICAL - valid JSON only: never use a literal double-quote character (") inside any string value. If you want to reference an exact phrase from a source, paraphrase it instead of quoting it directly - do not put it in quotation marks at all. A single unescaped double-quote anywhere in a string value breaks the entire JSON response and loses ALL data for this batch.
...
def call_claude(prompt: str, max_uses: int = 20, _json_retries: int = 2) -> dict:
    """
    ...
    for attempt in range(max_continuations):
        ...
        try:
            ...
            print(json_candidate[-800:])
            # Usually a stray unescaped quote inside a text field (e.g. a
            # quoted central bank line in Institutional Analysis) - this is a
            # probabilistic generation slip, not a deterministic bug, so a
            # fresh call (new conversation, same prompt) has a good chance of
            # succeeding even though the prompt itself now warns against it.
            # This is what previously took AUD's entire weekly batch down.
            if _json_retries > 0:
                print(f"[F4P Weekly Update] Retrying with a fresh call "
                      f"({_json_retries} retr{'y' if _json_retries == 1 else 'ies'} left)...")
                time.sleep(5)
                return call_claude(prompt, max_uses=max_uses, _json_retries=_json_retries - 1)
            print("[F4P Weekly Update] Out of retries - this batch genuinely failed this week.")
            raise
